...
export type FtpDeployInput = {
  /**
   * Folder on the FTP server, such as `/sites` or `/var/www`.
   * The function normalizes the value even when it has no trailing slash.
   */
  remoteRootDir: string;
  /**
   * Optional subdomain name, such as `acme`.
   * An empty value falls back to `remoteRootDir`.
   */
  subdomain?: string;
};
