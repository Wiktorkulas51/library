...
describe('minimalny kontrakt Fade In Text', () => {
	it('nie używa jawnych data-motion-sequence w blokach (motion jest globalny)', () => {
		...
		// The local animation was removed. Motion is handled by the global auto-reveal.
	...
	it('używa jednorazowego observera viewportu bez scroll listenera', () => {
		...
		// Why: HeroSplitBlock uses the global section auto-reveal,
		...
		// Why: main does not use transition:animate in this layout.
		// ClientRouter replaces the main DOM without a fade and resets scrolling.
