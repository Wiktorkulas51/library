...
describe('Data Integrity & Polish Naming Tests', () => {
  ...
  it('seo.json should have valid SEO meta data', () => {
    ...
    // (company data SSOT), while SEO contains only SEO settings.
...
// config Sveltia and the section register never went away. When config.yml doesn't
// exists, YAML tests are there
...
describe('CMS Config Integrity', () => {
  ...
  it.skipIf(!configExists)('config.yml parsuje się jako YAML', () => {
  ...
  it.skipIf(!configExists)('pliki JSON z global i navigation mają wpis w config.yml', () => {
    ...
    // The CMS intentionally skips technical files because this is AI configuration,
      // not content edited by the client.
