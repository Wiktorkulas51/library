...
let excludedCount = 0;
for (const relativePath of BUILD_SCOPE.exclude ?? []) {
  const target = path.resolve(dist, relativePath);
  const relativeTarget = path.relative(dist, target);

  // Konfiguracja buildu nie może usuwać plików poza katalogiem dist.
...
if (excludedCount > 0) {
  console.log(`prune-assets: usunięto ${excludedCount} wykluczonych ścieżek z dist/`);
}
