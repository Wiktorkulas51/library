...
/**
 * Sveltia CMS content overview: what the customer sees in the panel.
 * Reads the versioned public/admin/config.yml and prints the structure:
 * collections -> files -> fields (label, name, widget).
 *
 * Usage: npm run cms:list
 *
 * Before running, make sure the config is valid (npm run cms:check).
 */

const CONFIG_PATH = path.join(process.cwd(), 'public/admin/config.yml');

if (!fs.existsSync(CONFIG_PATH)) {
  console.error('Brak public/admin/config.yml. Dodaj konfigurację Sveltia CMS.');
  process.exit(1);
}
...
console.log(`\n📦 Sveltia CMS, zawartość panelu (${collections.length} kolekcji)\n`);
