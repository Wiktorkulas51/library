...
// Uses ffmpeg from @ffmpeg-installer/ffmpeg. For each file:
//   - wykrywa wysokosc (crf 26 gdy <=1280, inaczej crf 28)
//   - skaluje tylko w dol (max szerokosc 1920), zachowujac proporcje
//   - converts webm to mp4 and removes the source after successful conversion
// Errors for one file do not stop the complete operation.
...
// Recursively collects video files from the target directory.
...
for (const file of files) {
  ...
  try {
    ...
    // webm -> mp4 uses the final name, while mp4 uses a temporary file in the same directory.
    ...
    // An existing file can cause EPERM, so remove the original first.
