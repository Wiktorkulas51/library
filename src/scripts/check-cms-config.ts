...
// (src/data/*.json), versioned Sveltia config (public/admin/config.yml)
// and the section registry (src/config/section-registry.ts). Run through
...
//
// How it works: the config is versioned as a normal YAML file. This script
// validates its syntax and coverage without rewriting it.
...
// Katalogi skanowane podczas sprawdzania pokrycia konfiguracji CMS.
...
// Keys that should use the image widget in the CMS configuration.
...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// Step 1: config parsability (validation A)
// ---------------------------------------------------------------------------
function loadConfig(): any | null {
  console.log('Krok 1/5: parsowanie public/admin/config.yml...');
  if (!fs.existsSync(CONFIG_PATH)) {
    reportError(`Brak pliku ${CONFIG_PATH}. Dodaj konfigurację Sveltia CMS.`);
    return null;
  }
  try {
...
function checkFileCoverage(config: any): void {
  console.log('Step 2/5: consistency of JSON files on disk with config.yml...');
    ...
    if (file.includes('/pages/')) {
      reportWarning(
        `${file} has no entry in the CMS. This is intentional: pages are configured with ` +
          'page configami przez AI, nie przez Sveltia.',
      );
    } else {
      reportWarning(
        `${file} has no entry in the CMS. If a section is intentionally unused, ` +
          'that is OK. Otherwise, fill in the data and run npm run cms:check.',
      );
    }
  }
...
function checkPageSections(): void {
  console.log('Step 3/5: sections from page configs vs SECTION_REGISTRY...');
...
// ---------------------------------------------------------------------------
// Sveltia rejects a collection whose file has an empty fields list ([]), therefore
...
function checkFieldsAndWidgets(config: any): void {
  console.log('Step 4/5: empty field collections and image widgets...');
  let emptyCount = 0;
  let imageRefs = 0;

  for (const collection of config?.collections ?? []) {
    for (const file of collection.files ?? []) {
      // Validation D: empty fields list corrupts the entire Sveltia config
      if (hasEmptyFields(file.fields)) {
        emptyCount += 1;
        reportError(
          `The file "${file.file}" has an empty fields list. Sveltia rejects such a configuration. ` +
...
function checkHardcodedTexts(): void {
  console.log('Krok 5/5: hardcoded teksty w komponentach (informacyjnie)...');
  const files = fg.sync(
...
function printSummary(): void {
  ...
  if (errors.length > 0) {
    ...
    console.error('Co robić: napraw błędy powyżej, potem uruchom npm run cms:check ponownie.');
    process.exitCode = 1;
  } else {
    console.log(
