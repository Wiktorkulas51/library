...
function renderButtonHtml(props: {
  ...
  const sizeClasses: Record<ButtonSize, string> = {
    ...
    icon: 'size-10 sm:size-11 p-0 [&_svg]:size-4.5',
  };

  const classes = [
    'ui-button ui-type-cta-label group inline-flex items-center justify-center gap-1.5 rounded-md whitespace-nowrap transition-all font-bold',
  ...
  const content = size === 'icon'
    ? props.children || ''
    : `<span class="ui-button-text-wrap"><span class="ui-button-text-default">${props.children || ''}</span><span class="ui-button-text-hover" aria-hidden="true">${props.children || ''}</span></span>`;
...
describe('Button — kontrakt renderowania (HTML)', () => {
  ...
  describe('slot zawartosci', () => {
    ...
    it('renderuje zawartosc bez wrapa dla rozmiaru ikonowego', () => {
      const html = renderButtonHtml({
        size: 'icon',
        children: '<i class="ph ph-image" aria-hidden="true"></i>',
      });
      const $ = cheerio.load(html);
      expect($('.ui-button-text-wrap').length).toBe(0);
