...
// The filename without its extension is the lookup key because
// section-registry refers to components by filename.
...
const registryMap: Record<string, any> = Object.fromEntries(
  Object.entries(COMPONENT_MANIFEST).map(([name, entry]) => {
    ...
    if (explicitPath) {
      ...
      // Search the complete registry recursively by filename.
