...
2. **Configure company data** — edit `src/data/global/company.json` and `site.config.mjs`
...
| Site configuration (domain, template, BUILD_SCOPE, locale) | `site.config.mjs` |
...
- `.docs/CMS_STRUCTURE.md` — Sveltia CMS + config.yml generation
...
| `npm run cms:check` | Validate Sveltia CMS config.yml and data coverage |
...
- Each block in `SECTION_REGISTRY` has a group from `src/config/section-registry/types.ts`, variants and hint
...
### SEO Crawl
- `npm run qa:seo:crawl` runs the local SEO crawler against the running preview server. Use `--base-url` when the server is not on the default address, for example `npm run qa:seo:crawl -- --base-url http://[::1]:4322/ --max-pages 100`.
- The crawler checks sitemap, robots.txt, HTTP responses, title, meta description, canonical, robots, H1 structure, hreflang, JSON-LD, image `alt` attributes, duplicate metadata, internal links and orphan pages.
- Reports are written to `audit-reports/seo-crawl/latest.json`, `latest.md` and `latest.csv`. A non-zero exit code means that a blocking error was found. Warnings remain visible in the report.
- Read `.docs/SEO_CRAWL.md` before changing the crawler or relying on its report in QA.

### Routing
- `src/pages/[...page].astro` — catch-all route dla stron z `src/data/pages/`
- `src/pages/blog/` — osobne trasy listy i wpisów blogowych
- `src/pages/dev/` — biblioteka komponentów tylko lokalnie
- `src/pages/qa/` — fixture'y do kontroli jakości
- `src/layouts/Layout.astro` — globalny shell, Navbar i Footer

### Central Project Configuration
- `site.config.mjs` — URL, aktywny template, BUILD_SCOPE oraz ustawienia locale
- `src/config/template.ts` — konfiguracja istniejących danych template'u
...
src/pages/[...page].astro → strony z konfiguracji src/data/pages/
src/pages/blog/     → osobne trasy bloga
...
- `src/data/global/` — tagline, descriptions and other editable global content
...
6. **Tailwind first** — use Tailwind for layout, spacing, breakpoints and one-off styling; use CSS only for tokens, shared systems, pseudo-elements and complex animations
7. **Run `npm run check:atomic`** after every component change
8. **Reuse before Create** — check the table in section 3 before creating a new component
9. **Trailing slash /** for all internal links in Astro (use `formatInternalLink()` from `@utils/url`)
10. **Studio Compatibility** — blocks in registry must be registered in the manifest and `SECTION_REGISTRY`
11. **When adding a new block** → add the component manifest entry, domain registry variant and data JSON
12. **Comments in Astro:** inside JSX expressions (`{...}`) use JSX comments `{/* */}` or `/* */`, NOT `<!-- -->` — HTML comment inside expression breaks Astro compilation (CompilerError: Unexpected token)
13. **Code comment language — English only:** all comments in source files (`*.ts`, `*.js`, `*.mjs`, `*.astro`, `*.css`) must be written **exclusively in English** (EN). This applies to line comments `//`, block comments `/* */`, JSX `{/* */}` and JSDoc. Polish in code comments is forbidden.
14. **Locale determines content language:** `SITE_LOCALE` in `site.config.mjs` decides the language of generated content. When `I18N_CONFIG.enabled` is false, generate everything only in `SITE_LOCALE` and do not create locale folders, translated routes or switchers.
...
- A new block with media must be added to `component-manifest.ts` and the appropriate `SECTION_REGISTRY` domain module, and its example media must go to the section JSON.
...
| CMS | auth.php on server | Sveltia CMS + GitHub |
